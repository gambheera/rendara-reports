...
test.describe('viewer page navigation', () => {
  ...
  test('starts on page 1 with prev disabled and a multi-page document', async ({ page }) => {
    const status = page.locator('.rdr-viewer-status');
  ...
  test('next / prev move the visible page', async ({ page }) => {
    ...
    await expect(page.locator('.rdr-viewer-status')).toHaveText(/^Page 2 of \d+$/);
    await expect(page.getByRole('button', { name: 'Previous page' })).toBeEnabled();

    await page.getByRole('button', { name: 'Previous page' }).click();
    await expect(page.locator('.rdr-viewer-status')).toHaveText(/^Page 1 of \d+$/);
  });
  ...
  test('goto input jumps to a typed page', async ({ page }) => {
    ...
    await goto.fill('3');
    await goto.blur();
    await expect(page.locator('.rdr-viewer-status')).toHaveText(/^Page 3 of \d+$/);
  });
