...
/**
 * Grouped data-table renderer visual-regression snapshot (E4-S3, QA: "visual
 * snapshots for plain + grouped tables"). A compact A4-landscape page with a
 * grouped table — two region groups, each with a full-width header label, two
 * detail rows, and a subtotal footer band, plus a grand-total column footer —
 * rendered through the same engine + shared serializer as the other fixtures and
 * committed to `__fixtures__/grouped-table-page.html` (`golden-page-html.spec.ts`
...
<html lang="en">
  ...
  <body>
    <div class="stage" data-testid="grouped-table-page">${pageHtml}</div>
  </body>
</html>`;

test.describe('Renderer grouped-table visual regression (E4-S3)', () => {
  test('renders group header labels, subtotal footers and a grand total', async ({ page }) => {
